...
QObject * RSimpleBuildingResponseModel::getSa() const
{
    return theSa;
}

void RSimpleBuildingResponseModel::setSa(QObject *value)
{
...
//{
//    return parameterListToString(theSaList);
//}

//void RSimpleBuildingResponseModel::setIntensityList(QString value)
//{
//    theSaList = stringToParameterList(value);
//}

int RSimpleBuildingResponseModel::getNumberOfStoreys() const
{
    return theNumberOfStoreys;
}

void RSimpleBuildingResponseModel::setNumberOfStoreys(int value)
{
    theNumberOfStoreys = value;
}


QObject * RSimpleBuildingResponseModel::getLocation() const
{
    return theLocation;
}

void RSimpleBuildingResponseModel::setLocation(QObject *value)
{
    theLocation = qobject_cast<RParameter *>(value);
}


QString RSimpleBuildingResponseModel::getBuildingType()
{
    return theBuildingType;
}

void RSimpleBuildingResponseModel::setBuildingType(QString value)
{
    theBuildingType = value;
}


QString RSimpleBuildingResponseModel::getCodeDesignLevel()
{
    return theCodeDesignLevel;
}

void RSimpleBuildingResponseModel::setCodeDesignLevel(QString value)
{
...
int RSimpleBuildingResponseModel::evaluateModel()
{
...
//    double epsilon1 = sigma1 * rand()/RAND_MAX;
//    double epsilon2 = sigma2 * rand()/RAND_MAX;
   ...
   // qCritical() << "the Sa:" << Sa;
    ...
    double alpha = 1.0;
    if (theCodeDesignLevel == QString("Pre")){
        alpha = 1.0;
    }
    ...
    else if (theCodeDesignLevel == QString("High")){
        alpha = 4.0;
    }

    double H = 3.0 * theNumberOfStoreys;
...
//    double logAcc = -responseTheta16 * log(deltaY) + responseTheta17 * log(V) - responseTheta18 * log(Mu) + responseTheta19 * log(Kappa) + responseTheta20 * log(Sa) - responseTheta21 + epsilon2;
    ...
    double T = exp(-theResponseTheta1->getMeanValue()) * pow(H, theResponseTheta2->getMeanValue());
    double V = exp(-theResponseTheta3->getMeanValue()) * exp(-theResponseTheta4->getMeanValue() * H) * (2.0 + (alpha - 2.0) * (alpha - 1.0)) / 8.0;
    double gg = 9.81; //Gravity
    double deltaY = (T / 2.0 / 3.14159265359)*(T / 2.0 / 3.14159265359) * V * gg / H;
    double Mu = exp(theResponseTheta5->getMeanValue()) * pow(H, -theResponseTheta6->getMeanValue()) * (10.0 + (alpha - 2.0) * (alpha - 1.0)) / 16.0;
    double deltaU = Mu * deltaY;
    double Kappa = exp(-theResponseTheta7->getMeanValue() * Sa * pow(alpha, -theResponseTheta8->getMeanValue()));
    double logDrift = theResponseTheta9->getMeanValue() * log(deltaY) + theResponseTheta10->getMeanValue() * log(deltaU) - theResponseTheta11->getMeanValue() * log(V) - theResponseTheta12->getMeanValue() * log(Kappa) + theResponseTheta13->getMeanValue() * log(Sa) + theResponseTheta14->getMeanValue() * Sa - theResponseTheta15->getMeanValue() + epsilon1;
    double logAcc = -theResponseTheta16->getMeanValue() * log(deltaY) + theResponseTheta17->getMeanValue() * log(V) - theResponseTheta18->getMeanValue() * log(Mu) + theResponseTheta19->getMeanValue() * log(Kappa) + theResponseTheta20->getMeanValue() * log(Sa) - theResponseTheta21->getMeanValue() + epsilon2;
