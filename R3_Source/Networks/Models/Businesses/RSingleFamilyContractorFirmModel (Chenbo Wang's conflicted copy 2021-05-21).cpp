...
//    if(theInterventionApproachID>20 | theInterventionApproachID<0){
//        theInterventionApproachID = 0;
//    }

    // the number of housing units reserved for accomodating out-of-town workers
    // initial accomodation capacity immediately after a disaster
    theRemainingAccomodationCapacity = 0;

    // Define the random number of workers available in this scenario - remove this after paper
    theNumberOfSingleFamilyCrews = theAvailableWorkersPreDisaster;// + samplingMultiplier * theAvailableWorkersPreDisaster;

    //double rngSF = rand()/static_cast<double>(RAND_MAX);
    //theNumberOfSingleFamilyCrews = theAvailableWorkersPreDisaster + 2.0*rngSF * theAvailableWorkersPreDisaster;
    ...
    //theNumberOfSingleFamilyWorkersWaitingToBeAllocated = theNumberOfSingleFamilyCrews;

    //samplingMultiplier += 0.2;//initialized to be zero
    //qCritical()<< "the samplingMultiplier is: "<< samplingMultiplier<< "; Number of workers available in this scenario: " << theNumberOfSingleFamilyCrews ;

    return 0;
}


int RSingleFamilyContractorFirmModel::getBaselineNumberOfSingleFamilyCrews()
{
    return theAvailableWorkersPreDisaster;
}

void RSingleFamilyContractorFirmModel::setBaselineNumberOfSingleFamilyCrews(int value)
{
    theAvailableWorkersPreDisaster = value;
}


double RSingleFamilyContractorFirmModel::getBaseHourlyWages()
{
    return theBaseHourlyWages;
}

void RSingleFamilyContractorFirmModel::setBaseHourlyWages(double value)
{
...
{
    return theMinimumRatioOfDemandSatisfied;//should be between 0 and 1
}

void RSingleFamilyContractorFirmModel::setDemandRatio(double value)
{
    theMinimumRatioOfDemandSatisfied = value;
}

int RSingleFamilyContractorFirmModel::getInterventionID()
{
    return theInterventionApproachID;
}

void RSingleFamilyContractorFirmModel::setInterventionID(int value)
{
...
int RSingleFamilyContractorFirmModel::getNumberOfSingleFamilyCrews()
{
    return theNumberOfSingleFamilyCrews;
}

QVector<int> RSingleFamilyContractorFirmModel::getAllocatedSFCrewsVector()
{
    return theAllocatedWorkersVector;
}

QVector<int> RSingleFamilyContractorFirmModel::getAvailableSFCrewsVector()
{
    return theAvailableWorkersVector;
}

QVector<int> RSingleFamilyContractorFirmModel::getTotalDemandSFCrewsVector()
{
    return theTotalDemandForWorkersVector;
}

QVector<int> RSingleFamilyContractorFirmModel::getWaitlistSFCrewsVector()
{
    return theNumberOfDwellingsWaitingVector;
}

QVector<int> RSingleFamilyContractorFirmModel::getRemainingAccomodationSFCrewsVector()
{
    return theRemainingAccomodationCapacityVector;
}
...
int RSingleFamilyContractorFirmModel::evaluateModel()
{
    ...
    bool UseRevisedModel = true;
    if (UseRevisedModel == true){
        //qCritical()<< "The revised demand-supply relationship is used!";
        //double theMinimumRatioOfDemandSatisfied = theMultiplier;//should be between 0 and 1
        ...
        //----------------------------------------------------------------------------------------
        // increase the accommodation capacity by 500 monthly for 6 months starting 60 days after the disaster
        if (theInterventionApproachID == 1){
            if (theCurrentTime->getCurrentValue() - theTimeOfEvent <= 225 && theCurrentTime->getCurrentValue() - theTimeOfEvent >= 60){
                theRemainingAccomodationCapacity += 250;
            }
        }
        // increase the accommodation capacity by 1000 monthly for 6 months starting 60 days after the disaster
        else if(theInterventionApproachID == 2){
            if (theCurrentTime->getCurrentValue() - theTimeOfEvent <= 225 && theCurrentTime->getCurrentValue() - theTimeOfEvent >= 60){
                theRemainingAccomodationCapacity += 500;
            }
        }
        // increase the accommodation capacity by 1000 monthly for 3 months starting 60 days after the disaster
        else if(theInterventionApproachID == 3){
            if (theCurrentTime->getCurrentValue() - theTimeOfEvent <= 135 && theCurrentTime->getCurrentValue() - theTimeOfEvent >= 60){
                theRemainingAccomodationCapacity += 500;
            }
        }
        // increase the accommodation capacity by 2000 monthly for 3 months starting 60 days after the disaster
        else if(theInterventionApproachID == 4){
            if (theCurrentTime->getCurrentValue() - theTimeOfEvent <= 135 && theCurrentTime->getCurrentValue() - theTimeOfEvent >= 60){
                theRemainingAccomodationCapacity += 1000;
            }
        }
        // increase the accommodation capacity by 500 monthly for 6 months starting 60 days after the disaster (1000 in the first 60 days)
        else if(theInterventionApproachID == 5){
            if (theCurrentTime->getCurrentValue() - theTimeOfEvent < 60){
                theRemainingAccomodationCapacity = 1000;
            } else if (theCurrentTime->getCurrentValue() - theTimeOfEvent <= 225 && theCurrentTime->getCurrentValue() - theTimeOfEvent >= 60){
                theRemainingAccomodationCapacity += 250;
            }
        }
        // increase the accommodation capacity by 1000 monthly for 6 months starting 60 days after the disaster (1000 in the first 60 days)
        else if(theInterventionApproachID == 6){
            if (theCurrentTime->getCurrentValue() - theTimeOfEvent < 60){
                theRemainingAccomodationCapacity = 1000;
            } else if (theCurrentTime->getCurrentValue() - theTimeOfEvent <= 225 && theCurrentTime->getCurrentValue() - theTimeOfEvent >= 60){
                theRemainingAccomodationCapacity += 500;
        ...
        // increase the accommodation capacity by 1000 monthly for 3 months starting 60 days after the disaster (1000 in the first 60 days)
        else if(theInterventionApproachID == 7){
            if (theCurrentTime->getCurrentValue() - theTimeOfEvent < 60){
                theRemainingAccomodationCapacity = 1000;
            } else if (theCurrentTime->getCurrentValue() - theTimeOfEvent <= 135 && theCurrentTime->getCurrentValue() - theTimeOfEvent >= 60){
                theRemainingAccomodationCapacity += 500;
            }
        }
        // increase the accommodation capacity by 2000 monthly for 3 months starting 60 days after the disaster (1000 in the first 60 days)
        else if(theInterventionApproachID == 8){
            if (theCurrentTime->getCurrentValue() - theTimeOfEvent < 60){
                theRemainingAccomodationCapacity = 1000;
            } else if (theCurrentTime->getCurrentValue() - theTimeOfEvent <= 135 && theCurrentTime->getCurrentValue() - theTimeOfEvent >= 60){
                theRemainingAccomodationCapacity += 1000;
            }
        // theRemainingAccomodationCapacity stays 1000
        }else if(theInterventionApproachID == 9){
            if (theCurrentTime->getCurrentValue() - theTimeOfEvent < 60){
                theRemainingAccomodationCapacity = 1000;
            }
        // theRemainingAccomodationCapacity stays 2000
        }else if(theInterventionApproachID == 10){
            if (theCurrentTime->getCurrentValue() - theTimeOfEvent < 60){
                theRemainingAccomodationCapacity = 2000;
            }
        // provides no accomodation at all (no new workers)
        }else if(theInterventionApproachID == 999){
            if (theCurrentTime->getCurrentValue() - theTimeOfEvent < 60){
                theRemainingAccomodationCapacity = 0;
            }
        // Skip the need for accomodation
        }else if (theInterventionApproachID == 9999){
            theRemainingAccomodationCapacity = 9999999999;

        }else{
            qCritical()<<"Please specify the intervention approach for SF crews' accommodation!";
        }

        // if demand*ratio > supply, then bring in more workers
        if (theMinimumRatioOfDemandSatisfied * (theNumberOfDwellingsWaiting + theNumberOfSingleFamilyWorkersCurrentlyAllocated) > (theNumberOfSingleFamilyWorkersWaitingToBeAllocated + theNumberOfSingleFamilyWorkersCurrentlyAllocated)){

            double theGapBetweenSupplyAndDemand = theMinimumRatioOfDemandSatisfied * (theNumberOfDwellingsWaiting + theNumberOfSingleFamilyWorkersCurrentlyAllocated) - (theNumberOfSingleFamilyWorkersWaitingToBeAllocated + theNumberOfSingleFamilyWorkersCurrentlyAllocated);
        ...
        }else if(theMinimumRatioOfDemandSatisfied * (theNumberOfDwellingsWaiting + theNumberOfSingleFamilyWorkersCurrentlyAllocated) < (theNumberOfSingleFamilyWorkersWaitingToBeAllocated + theNumberOfSingleFamilyWorkersCurrentlyAllocated)) {
            double theGapBetweenSupplyAndDemand = (theNumberOfSingleFamilyWorkersWaitingToBeAllocated + theNumberOfSingleFamilyWorkersCurrentlyAllocated) - theMinimumRatioOfDemandSatisfied * (theNumberOfDwellingsWaiting + theNumberOfSingleFamilyWorkersCurrentlyAllocated);
            // assume a fraction of theGapBetweenSupplyAndDemand number of out-of-town workers will leave when the target is met
            theRatioOfOutOfTownWorkersWhoLeave = 0.5;
            // check if the supply is still larger than baseline supply if a fraction of workers are to leave
            if ((theNumberOfSingleFamilyWorkersWaitingToBeAllocated + theNumberOfSingleFamilyWorkersCurrentlyAllocated) - theRatioOfOutOfTownWorkersWhoLeave * theGapBetweenSupplyAndDemand >  theAvailableWorkersPreDisaster){
                theRemainingAccomodationCapacity += theRatioOfOutOfTownWorkersWhoLeave * theGapBetweenSupplyAndDemand;
                theNumberOfSingleFamilyWorkersWaitingToBeAllocated -= theRatioOfOutOfTownWorkersWhoLeave * theGapBetweenSupplyAndDemand;
