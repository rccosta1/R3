...
RPublicMoneyLenderModel::RPublicMoneyLenderModel(QObject *parent, QString name)
...
RPublicMoneyLenderModel::~RPublicMoneyLenderModel()
{

}


int RPublicMoneyLenderModel::resetTime()
{
    timeOfEvent = 0.0;
...
double RPublicMoneyLenderModel::getMedianDelayForPayments()
{
    return theMedianDelayForPayments;
}

void RPublicMoneyLenderModel::setMedianDelayForPayments(double value)
{
    theMedianDelayForPayments = value;
}


double RPublicMoneyLenderModel::getDispersionOfDelayForPayments()
{
    return theDispersionOfDelayForPayments;
}

void RPublicMoneyLenderModel::setDispersionOfDelayForPayments(double value)
{
    theDispersionOfDelayForPayments = value;
}


double RPublicMoneyLenderModel::getDelayMultiplierForSmallRequests()
{
    return theDelayMultiplierForSmallRequests;
}


void RPublicMoneyLenderModel::setDelayMultiplierForSmallRequests(double value)
{
    theDelayMultiplierForSmallRequests = value;
}


double RPublicMoneyLenderModel::getSmallClaimThreshold()
{
    return theSmallClaimThreshold;
}

void RPublicMoneyLenderModel::setSmallClaimThreshold(double value)
{
    theSmallClaimThreshold = value;
}


double RPublicMoneyLenderModel::getDelayForOwnerOccupiedBuildings()
{
    return theDelayForOwnerOccupied;
}

void RPublicMoneyLenderModel::setDelayForOwnerOccupiedBuildings(double value)
{
    theDelayForOwnerOccupied = value;
}

double RPublicMoneyLenderModel::getDelayForRenterOccupiedBuildings()
{
    return theDelayForRenterOccupied;
}

void RPublicMoneyLenderModel::setDelayForRenterOccupiedBuildings(double value)
{
    theDelayForRenterOccupied = value;
}


void RPublicMoneyLenderModel::getRequests()
{

}


int RPublicMoneyLenderModel::evaluateModel()
{
    // Output
    ...
    for (int i = 0; i < theAllRequestList.size(); i++){
        ...
        if(theAllRequestList.at(i)->getContentType().contains("PublicMoney") && theAllRequestList.at(i)->getRequestedAmountList().size() > 0){
    ...
    for (int i = 0; i < theRequestDestinationList.size(); i++){
        ...
        for (int j = 0; j < theRequestedAmountList.at(i).size(); j++){
            ...
            theDelaysForEachBuilding << theProgramStartDelay + 7.0 * theDelayMultiplier * exp(theNormalDistribution.getInverseNormalCDF(theUx,1.0) * sigma + mu);
