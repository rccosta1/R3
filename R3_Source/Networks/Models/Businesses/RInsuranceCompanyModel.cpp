...
RInsuranceCompanyModel::RInsuranceCompanyModel(QObject *parent, QString name)
    : RNetworkModel(parent, name)
{
    thePathMatrixNeed = false;

    theDelayMultiplierSmallClaims = 0.75;
    theDelayMultiplierLargeClaims = 1.25;
    theSmallClaimThreshold = 0.25;
    theLargeClaimThreshold = 0.75;
...
RInsuranceCompanyModel::~RInsuranceCompanyModel()
{

}


int RInsuranceCompanyModel::resetTime()
{
...
double RInsuranceCompanyModel::getMedianDelayForPayments()
{
    return theMedianDelayForPayments;
}

void RInsuranceCompanyModel::setMedianDelayForPayments(double value)
{
    theMedianDelayForPayments = value;
}


double RInsuranceCompanyModel::getDispersionOfDelayForPayments()
{
    return theDispersionOfDelayForPayments;
}

void RInsuranceCompanyModel::setDispersionOfDelayForPayments(double value)
{
    theDispersionOfDelayForPayments = value;
}


double RInsuranceCompanyModel::getDelayMultiplierForSmallClaims()
{
    return theDelayMultiplierSmallClaims;
}

void RInsuranceCompanyModel::setDelayMultiplierForSmallClaims(double value)
{
    theDelayMultiplierSmallClaims = value;
}


double RInsuranceCompanyModel::getDelayMultiplierForLargeClaims()
{
    return theDelayMultiplierLargeClaims;
}

void RInsuranceCompanyModel::setDelayMultiplierForLargeClaims(double value)
{
    theDelayMultiplierLargeClaims = value;
}


double RInsuranceCompanyModel::getSmallClaimThreshold()
{
    return theSmallClaimThreshold;
}

void RInsuranceCompanyModel::setSmallClaimThreshold(double value)
{
    theSmallClaimThreshold = value;
}


double RInsuranceCompanyModel::getLargeClaimThreshold()
{
    return theLargeClaimThreshold;
}

void RInsuranceCompanyModel::setLargeClaimThreshold(double value)
{
    theLargeClaimThreshold = value;
}




void RInsuranceCompanyModel::getRequests()
{
...
int RInsuranceCompanyModel::evaluateModel()
{
    // Output
    ...
    for (int i = 0; i < theAllRequestList.size(); i++){
        ...
        if(theAllRequestList.at(i)->getContentType().contains("InsuranceMoney") && theAllRequestList.at(i)->getRequestedAmountList().size() > 0){
            ...
            //qCritical() << this->objectName() << "is taking this request" << theAllRequestList.at(i)->objectName();
                ...
                theRequesterBuildingValue << theAllRequestList.at(i)->getRequesterBuildingValue();
        }
    }
    ...
    for (int i = 0; i < theRequestDestinationList.size(); i++){
        ...
        thePacketBuildingValue << theRequesterBuildingValue.at(i);
        ...
        for (int j = 0; j < theRequestedAmountList.at(i).size(); j++){
            ...
            if (theRequestedAmountList.at(i).at(j) / theRequesterBuildingValue.at(i).at(j) < theSmallClaimThreshold){
                theDelayMultiplier = theDelayMultiplierSmallClaims;

            } else if(theRequestedAmountList.at(i).at(j) / theRequesterBuildingValue.at(i).at(j) > theLargeClaimThreshold){
                theDelayMultiplier = theDelayMultiplierLargeClaims;
            }

            double theUx = rand()/static_cast<double>(RAND_MAX);
            theDelaysForEachBuilding << 7.0 * theDelayMultiplier * exp(theNormalDistribution.getInverseNormalCDF(theUx,1.0) * sigma + mu);
    ...
    for (int i = 0; i < indexesToRemove.size(); i++){
        ...
        thePacketBuildingValue.removeAt(indexesToRemove.at(i));
    }
