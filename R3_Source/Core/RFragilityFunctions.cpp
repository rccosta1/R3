...
RFragilityFunctions::RFragilityFunctions(RDomain *theDomain)
    : RMethod(theDomain, 0)
{
...
double RFragilityFunctions::getExpectedRepairTime(double desiredPercentage, QVector<double> damageProbabilities)
{
    // Create a normal distribution tool
    RNormalDistribution theNormalDistribution(theDomain);
    ...
    for (int i = 0; i < theRepairTimeMedians.count(); i++) {
        ...
        // Calculate the time until the desired restoration ratio
        double result = theRepairTimeMedians[i] + theNormalDistribution.getInverseNormalCDF(desiredPercentage, theRepairTimeStandardDeviations[i]);
