...
//Domain and class wide model iteration map
QMap<RModel*,int> RModelManager::modelIterationMap;


RModelManager::RModelManager(RDomain *theDomain)
    ...
    for (int i = 0; i < modelListList.count(); i++) {
        ...
        for (int j = 0; j < leveliModelList.count(); j++) {
            ...
            for (int k = 0; k < responseList.count(); k++) {
                ...
                modelMap.insert(responseModel, model);
            }
        }
        //
...
int RModelManager::plotFlowchart()
{
    ...
    theMainWindow->addSubWindow(widget, QString("Object Flowchart"), false);
...
int RModelManager::establishPaths()
{
    ...
    for (int i = 0; i < fromModels.count(); i++) {
        ...
        RNetworkModel *theNetworkModel = qobject_cast<RNetworkModel*>(fromModels.at(i));

        if (theNetworkModel && theNetworkModel->getPathMatrixNeed() == true){
            for (int j = 0; j < toModels.count(); j++) {
...
int RModelManager::setPaths()
{
    if (pathMatrix.size() == 0) {
        qCritical() << "[2] Cannot print paths until the pathmatrix is established. (Or perhaps there are no models?)";
...
int RModelManager::removeClosedPaths()
{
    ...
    for (int i = 0; i < networkComponents.count(); i++) {
        ...
        if (networkComponents.at(i) && networkComponents.at(i)->getFunctionality() == true) {
...
int RModelManager::setModelIterationNumber(RModel* model, int iterationNumber)  //SG add
{

    modelIterationMap.insert(model,iterationNumber);
...
int RModelManager::getModelIterationNumber(RModel* model)  //SG add
{

    int value = modelIterationMap.value(model);
    return value;
