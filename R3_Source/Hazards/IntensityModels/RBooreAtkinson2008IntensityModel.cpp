...
double RBooreAtkinson2008IntensityModel::getEpicenterLocationLatitude()
{
    RLocation *hypocentre = parameterToLocation(theHypocentreList[0]);
    return hypocentre->getLatitude();
}

double RBooreAtkinson2008IntensityModel::getEpicenterLocationLongitude()
{
    RLocation *hypocentre = parameterToLocation(theHypocentreList[0]);
    return hypocentre->getLongitude();
}
...
int RBooreAtkinson2008IntensityModel::evaluateModel()
{
    ...
    for (int index = 0; index < theMagnitudeList.count(); index++) {
        ...
        if (T > periodTableVector.last() || T < periodTableVector.first()) {
            ...
            //qCritical() << "Warning: The period" << T << "from the parameter" << thePeriodList[i]->objectName() << "entered in the model" << objectName() << "is out of the range of" << periodTableVector[1] << "and" << periodTableVector.last() << "sec.";
        ...
        for (int j = lowerIndex; j <= upperIndex; j++) { // At max, there will be 2 iterations
            ...
            double lnY = FM + FD + FS; // + sigmaT * epsilon  <<=========== HERE;
            lnYVector << lnY;
        ...
        spectralResponse += Y;
        //qDebug() << "The intensity is" <<  Y;
    }


    theResponse->setCurrentValue(spectralResponse);
    //qCritical() << " ================== " << spectralResponse;
    //    qCritical() << "";
    //    qCritical() << "";
    //    qCritical() << "";
    //    qCritical() << "";
    ...
    //    qCritical() << "";
    //    qCritical() << "";
    //    qCritical() << "Check line 520 in the" << this->objectName();
    //    qCritical() << "";
    //    qCritical() << "";
    //    qCritical() << "";
    //    qCritical() << "";
